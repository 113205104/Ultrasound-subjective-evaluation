(function () {
  ...
  // ➔ Bug fix: readRating() 永遠回傳物件（最差是 {}），直接 if(serverRating) 判斷會
  //    把每張圖都塞進 localMemoryDraft，導致儲存時對未作答的圖送出空白 payload 覆蓋舊答案。
  //    hasAnyAnswer 確保只有真正有填分數的物件才算有效草稿。
  function hasAnyAnswer(rating) {
